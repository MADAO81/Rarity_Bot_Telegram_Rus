"""
Планировщик для бота Рарити.
Отправка вдохновения в 9:30 и совета дня в 18:00.

Автор: MADAO81
Версия: 2.0 — DeepSeek для рассылок
"""
...
from bot.services.ai_service import get_daily_inspiration, get_daily_tip
...
async def subscribe_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "💎 *Ты подписался на ежедневные рассылки Рарити!*\n\n"
...
async def send_inspiration(app):
    ...
    if not active_chats:
        logger.info("📭 Нет активных чатов для вдохновения")
        return

    logger.info(f"💎 Отправка вдохновения в {len(active_chats)} чатов...")

    message = await get_daily_inspiration()
    if not message:
        message = "💎 *Бриллиантовое вдохновение:* Сегодня — идеальный день, чтобы добавить блеска в свою жизнь! ✨"
...
async def send_daily_tip(app):
    active_chats = get_active_chats()
    if not active_chats:
        logger.info("📭 Нет активных чатов для совета дня")
        return

    logger.info(f"🧵 Отправка совета дня в {len(active_chats)} чатов...")

    message = await get_daily_tip()
    if not message:
        message = "🧵 *Совет от Рарити:* У тебя есть старая рубашка? Не спеши её выбрасывать! Из рукавов можно сделать модные повязки для волос, а из воротника — оригинальный браслет. Твори! 🎀"
...
def stop_scheduler():
    try:
        scheduler.shutdown()
        logger.info("⏹️ Планировщик Рарити остановлен")
    except Exception as e:
